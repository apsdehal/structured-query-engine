...
class Indexer:
    ...
    def delete(self, doc_type, doc_id):
        ...
        try:
            ...
            if self.document_store[self.index][doc_type][self.generate_shard_number(doc_id)][doc_id]['is_deleted'] == False:
        ...
        if(doc_type not in self.index_doc_type):
            ...
            for field in self.mapping[doc_type]['properties']:
                if(self.mapping[doc_type]['properties'][field].get('index',True)):
                    self.tfTable[self.index][doc_type] = [ AutoVivification() for x in range(self.config['num_shards'])] # defaultdict(lambda : defaultdict(list))
    ...
    def generate_shard_number(self, doc_id):
        return doc_id % self.config['num_shards']
